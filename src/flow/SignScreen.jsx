...
export default function SignScreen({ doc, mySig, bio = false, signerName = 'ຜູ້ລົງນາມ', meId, onSaveSig, onDone, onOriginalSign, onBack }) {
  ...
  // ── E2/E13: ລາຍເຊັນຕົ້ນສະບັບ — ພິມ/ດາວໂຫລດ ທຸກໄຟລ໌ → ເຊັນມື → ສະແກນ → ອັບໂຫລດກັບ ຄົບທຸກໄຟລ໌ → OTP → ຢືນຢັນ (ບໍ່ມີ watermark, ບໍ່ວາງກ່ອງເຊັນ) ──
  const downloadOriginal = (i) => {
    const f = files[i]
    if (f?.file) { const url = URL.createObjectURL(f.file); const a = document.createElement('a'); a.href = url; a.download = f.name; a.click(); URL.revokeObjectURL(url) }
    else flash('ກຳລັງດາວໂຫລດ PDF...')
  }
  const pickOriginalFile = (i, e) => { const f = e.target.files?.[0]; if (f) setOriginalFiles((m) => ({ ...m, [i]: f })) }
    ...
    return (
      <div className="app">
        ...
        <div className="scroll sign-scroll">
          <div className="sign-ok"><span className="sign-ok-ic"><Icon.check /></span><div className="sign-ok-txt"><b>{successMsg}</b><p>{sigType === 'original' ? 'ໄຟລ໌ PDF ທີ່ເຊັນດ້ວຍມືໄດ້ຖືກອັບໂຫລດ ແທນທີ່ເອກະສານເດີມແລ້ວ' : 'ລາຍເຊັນຂອງທ່ານໄດ້ຖືກບັນທຶກໃສ່ເອກະສານແລ້ວ'}</p></div></div>
          {sigType === 'original'
            ? <div className="preview-frame"><PdfViewer files={files.map((f, i) => ({ name: f.name, file: originalFiles[i] }))} mode="preview" watermark={false} activeSignerId={null} placements={[]} signers={[]} onAdd={noop} onMove={noop} onRemove={noop} /></div>
    ...
    <div className="app">
      ...
      {stage === 'original' && (
        <div className="modal-overlay" onClick={() => setStage('choose')}>
          <div className="modal-sheet tall" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ padding: '8px 14px 18px', display: 'flex', flexDirection: 'column', gap: 14, maxHeight: '60vh', overflowY: 'auto' }}>
              <p className="muted" style={{ margin: 0, fontSize: 12.5 }}>1. ດາວໂຫລດ/ພິມເອກະສານ (ຄົບທຸກໄຟລ໌) → 2. ເຊັນດ້ວຍມືເທິງເຈ້ຍ → 3. ສະແກນເປັນ PDF ແລ້ວອັບໂຫລດກັບ (ຄົບທຸກໄຟລ໌) → 4. ຢືນຢັນ OTP</p>
              {files.map((f, i) => (
                <div className="orig-file-card" key={i}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <button className="sign-type" onClick={() => downloadOriginal(i)}>
                      ...
                      <div><b>ດາວໂຫລດ / ພິມ</b><span>ນຳໄປເຊັນດ້ວຍມື</span></div>
                    </button>
                    <input type="file" ref={(el) => (originalInputRefs.current[i] = el)} hidden accept="application/pdf" onChange={(e) => pickOriginalFile(i, e)} />
                    <button className="sign-type" onClick={() => originalInputRefs.current[i]?.click()}>
                      <span className="sign-type-ic lanit"><Icon.upload /></span>
                      <div><b>ອັບໂຫລດ PDF ທີ່ສະແກນແລ້ວ</b><span>{originalFiles[i] ? originalFiles[i].name : 'ຮັບສະເພາະໄຟລ໌ PDF'}</span></div>
                      {originalFiles[i] ? <Icon.checkCircle /> : <Icon.chevron />}
                    </button>
                    {originalFiles[i] && (
                      <button className="dd-cmt-reply" onClick={() => setOriginalPreviewIdx(originalPreviewIdx === i ? null : i)}>
                        <Icon.eye /> {originalPreviewIdx === i ? 'ເຊື່ອງ preview' : 'preview ໄຟລ໌ທີ່ອັບໂຫລດ'}
                      </button>
                    )}
      ...
      {stage === 'otp' && (
        <div className="modal-overlay">
          <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
            <div className="modal-head"><b><Icon.shield /> ຢືນຢັນຕົວຕົນ{otpFor === 'original' ? '' : ' LANIT'}</b><button className="icon-mini" onClick={() => setStage('choose')}><Icon.x /></button></div>
